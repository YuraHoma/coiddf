...
//   translated — падаємо на відсутньому перекладі (типово);
//   human      — падаємо ще й на машинному, не вичитаному людиною.
//
// STALE_HUMAN на рівні "translated" НЕ блокує свідомо: клієнт редагує
// контент у CMS сам, і його правка поверх вичитаного поля не має зупиняти
// публікацію всього сайту. Стара вивірена англійська публікується з
// попередженням і чекає перекладача. STALE_MACHINE лишається блокувальним:
// у CI перед збіркою його оновлює npm run i18n:translate, тож дійти до
// збірки він може лише при збої самого перекладу — і тоді падіння доречне.
const BLOCKING = {
  translated: [STATE.MISSING, STATE.STALE_MACHINE],
